...
const editor = {
  ...
  auction: async function () {
    if (this._auction === null) this._auction = await auctionService.auction();
    return this._auction;
  },
  setSettings: function (settings) {
    if (this._auction != null) this._auction.update(settings);
  },
  addUser: function (user) {
    if (this._auction?.includes(user) === false) this._auction.users.push(user);
  },
  removeUser: function (user) {
    if (this._auction?.includes(user) === true) this._auction.users = this._auction.filter(u => u !== user);
  },
  save: async function () {
...
async function saveSettings() {
  ...
  if (!(await editor.save())) alert('Не удалось сохранить настройки. Проверьте правильность заполнения полей');else window.location = '/';
}
