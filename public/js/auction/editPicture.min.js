...
const editor = {
  ...
  picture: async function () {
    if (this._picture == null) this._picture = await pictureService.findById(getPictureId());
  ...
  selectFilename: async function (filename) {
    ...
    if (this._picture == null) this._picture = await pictureService.findById(getPictureId());
    (_this$_picture = this._picture) === null || _this$_picture === void 0 ? void 0 : _this$_picture.update({
      filename: filename
    });
    redrawFiles();
  }
...
async function redrawFiles() {
  ...
  if (picture == null || files == null) return element.html(`<div class="uk-alert uk-alert-warning">Не удалось загрузить файлы</div>`);
  if (files.length === 0) return element.html(`<div class="uk-alert">Нет файлов</div>`);
  let html = "";
  files.forEach(file => {
...
async function refreshFields() {
  let picture = await editor.picture();
  if (picture == null) return alert('Не удалось загрузить данные');
...
async function save() {
  ...
  if (await pictureService.update(getPictureId(), picture)) window.location = '/';else alert('Не удалось сохранить данные');
}
...
$(document).ready(function () {
  ...
  $('#upload-image-button').click(function (e) {
    ...
    if (input.files.length === 0) return alert('Выберите файл для загрузки');
  ...
  $('#delete-picture').click(function (e) {
    if (confirm('Вы действительно хотите удалить картину?')) pictureService.remove(getPictureId()).then(success => {
      if (success) window.location = '/';else alert('Не удалось удалить картинку');
    });
  });
});