...
$(document).ready(function () {
  let update = async function () {
    ...
    if (auction == null || user == null || pictures == null) return element.html(`<div class="uk-alert uk-alert-warning">Не удалось загрузить данные</div>`);
    if (pictures.length === 0) return element.html(`<div class="uk-alert uk-alert-warning">Нет доступных картин</div>`);
    let html = "";
...
async function savePictures() {
  ...
  element.find('input[type="checkbox"]').each(function () {
    if (this.checked) pictures.push(this.name);
  });
  let auction = await auctionService.auction();
  auction.update({
    pictures: pictures
  });
  if (auctionService.save(auction)) window.location = '/';else alert("Что-то пошло не так. Проверьте что-нибудь");
}