...
$(document).ready(function () {
  let refresh = async () => {
    ...
    elements.html("");
    if (pictures === null || user == null) elements.html(`<div uk-alert class="uk-alert-warning">Не удалось загрузить данные</div>`);else if (pictures.length === 0) {
      elements.html(`<div uk-alert class="uk-alert-primary">Нет картин</div>`);
    } else {
      let html = "";
      pictures.forEach(picture => {
        let titleSuffix = "";
        if (user?.isadmin === true) titleSuffix = `
      ...
      if (user?.isadmin === true) html += `<a class="uk-button uk-button-default" href="/auction/picture/add">Добавить картину</a>`;
